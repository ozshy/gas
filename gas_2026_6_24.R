...
i2025_2.df = subset(indiv2025_1.df, select = c(id, ind_weight_all, income_hh, hhincome, age, cc_adopt, dc_adopt, cc_rewards, urban_cat))
#
...
m2025_4.df = subset(m2025_3.df, select = c(id, pi, amnt, cc_surcharge, income_hh, hhincome, age, urban_cat, cc_adopt, dc_adopt, discount, cc_rewards))
names(m2025_4.df)
...
#rename variables
m2025_8.df = m2025_7.df %>% rename("Amount" = "amnt", "Own_credit" = "cc_adopt", "Credit_reward" = "cc_rewards",  "Own_debit" = "dc_adopt", "Discount" = "discount", "HH_income" = "income_hh", "PI" = "pi", "Age" = "age", "Urban" = "urban_cat")
names(m2025_8.df)
...
cash.model = Paid_cash ~ HH_income + Age +Urban + Own_credit + Credit_reward + Own_debit + Amount + Discount #+ cc_surcharge
#
credit.model = Paid_credit ~ HH_income + Age +Urban + Own_credit + Credit_reward + Own_debit + Amount + Discount #+ cc_surcharge 
#
debit.model = Paid_debit ~ HH_income + Age +Urban + Own_credit + Credit_reward + Own_debit + Amount + Discount #+ cc_surcharge 
...
# model
pi.model = PI ~ HH_income + Age + Urban + Own_credit + Own_debit + Amount + Discount + Credit_reward
...
#Begin: Table 1: fraction PI by Urban\rural category####
...
# LaTeX table
print(xtable(urban.df, digits=1), include.rownames = F, hline.after = c(0))

# Info for Table 1: urban/rural
nrow(m2025_8.df)# num payments
...
#End: Table 1: fraction PI by Urban category####
