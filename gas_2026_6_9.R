...
i2025_2.df = subset(indiv2025_1.df, select = c(id, ind_weight_all, income_hh, hhincome, age, cc_adopt, dc_adopt, cc_rewards))
#
...
t2025_3.df = subset(t2025_2.df, select = c(id, pi, amnt, device, mobile_app, cc_discount, dc_rewards, accept_cash, accept_card, discount))
dim(t2025_3.df)
#
...
ggplot(frac.df, aes(x=year.vec, y=cash_frac_w.vec, group = 1)) + geom_line(linewidth = 1.0) +geom_point(size=1.9) + geom_line(aes(y=credit_frac_w.vec), linewidth = 1.0, linetype="dotdash", color="red") +geom_point(aes(y=credit_frac_w.vec), size=1.9, color="red") +geom_line(aes(y=debit_frac_w.vec), linewidth = 1.0, linetype = "longdash", color="blue") +geom_point(aes(y=debit_frac_w.vec), size=1.9, color="blue") + scale_y_continuous(breaks = seq(0, 0.5, 0.025), labels = percent) +labs(x="Year", y="Percentage of payment method used") +theme(axis.text.x = element_text(size = 14, color = "black"),  axis.text.y = element_text(size = 16, color = "black"), text = element_text(size = 20)) +annotate("text", x = 2.3, y = 0.24, label = "Cash", angle = -35,  color = "black", size = 6, hjust = 0) +annotate("text", x = 2, y = 0.355, label = "Credit card", angle = -7,  color = "red", size = 6, hjust = 0)  +annotate("text", x = 2, y = 0.385, label = "Debit card", angle = 38,  color = "blue", size = 6, hjust = 0) 

#End: Figure 1: Fraction of PI by year####

#+++++++++++++++

#Begin: Figure: Dollar spending on gas by year####
...
#End: Figure: Dollar spending on gas by year####
...
m2025_4.df = subset(m2025_3.df, select = c(id, pi, amnt, income_hh, hhincome, age, cc_adopt, dc_adopt, discount, cc_rewards))
...
# imputing missing "discount" values
...
m2025_5.df = missRanger(data = m2025_5.df, formula =  cc_adopt ~ income_hh + age)
with(m2025_5.df, table(pi, cc_adopt, useNA = "always"))
...
m2025_6.df = missRanger(data = m2025_6.df, formula =  cc_rewards ~ age + income_hh)
table(m2025_6.df$cc_adopt, m2025_6.df$cc_rewards, useNA = "always")

# add new variables: paycash, paycredit, paydebit all 0,1, 
...
cash.model = Paid_cash ~ income_hh + age + cc_adopt + cc_rewards + dc_adopt + amnt + discount 
credit.model = Paid_credit ~ income_hh + age + cc_adopt + cc_rewards + dc_adopt + amnt + discount 
debit.model = Paid_debit ~ income_hh + age + cc_adopt + cc_rewards + dc_adopt + amnt + discount 
...
(cash.reg = logitmfx(cash.model, data = m2025_7.df, atmean = F))

#cash regression R built-in glm yielding 0,1 warninig
...
(credit.reg = logitmfx(credit.model, data = m2025_7.df, atmean = F))

#credit regression R built-in glm yielding 0,1 warninig
...
(debit.reg = logitmfx(debit.model, data = m2025_7.df, atmean = F))
...
#texreg(cash.reg, credit.reg, debit.reg)# cannot do 3 together
# => manually cut and paste the 3 tables below into a LaTeX table
(cash_reg.df = data.frame(Variable = rownames(cash.reg$mfxest), Cash.Mar.Eff = round(cash.reg$mfxest[, "dF/dx"],3), Cash.Std.Err = round(cash.reg$mfxest[, "Std. Err."],3), Cash.P.Val = round(cash.reg$mfxest[, "P>|z|"],3), row.names = NULL))
#
#(reg1.df$Cash.stars = 
#    ifelse(is.na(reg1.df$Cash.P.Val), "",
#           ifelse(reg1.df$Cash.P.Val < 0.01, "***",
#                  ifelse(reg1.df$Cash.P.Val < 0.05, "**",
#                         ifelse(reg1.df$Cash.P.Val < 0.10, "*", #"")))))
# compare with texreg
texreg(cash.reg)

(credit_reg.df = data.frame(Variable = rownames(credit.reg$mfxest), Credit.Mar.Eff = round(credit.reg$mfxest[, "dF/dx"],3), Credit.Std.Err = round(credit.reg$mfxest[, "Std. Err."],3), Credit.P.Val = round(credit.reg$mfxest[, "P>|z|"],3), row.names = NULL))
# compare with texreg
texreg(credit.reg)

(debit_reg.df = data.frame(Variable = rownames(debit.reg$mfxest), Debit.Mar.Eff = round(debit.reg$mfxest[, "dF/dx"],3), Debit.Std.Err = round(debit.reg$mfxest[, "Std. Err."],3), Debit.P.Val = round(debit.reg$mfxest[, "P>|z|"],3), row.names = NULL))
# compare with texreg
texreg(debit.reg)

#combining the three data frames
(reg1.df = cbind(cash_reg.df, credit_reg.df[,-1], debit_reg.df[,-1]))
#rename variables column
(reg2.df = reg1.df %>% mutate(Variable = case_when(Variable == "income_hh" ~ "HH_income(10k)", Variable == "age" ~ "Age", Variable == "cc_adopt1" ~ "Own_credit", Variable == "dc_adopt1" ~ "Own_debit", Variable == "amnt" ~ "Amount", Variable == "discount1" ~ "Discount", Variable == "cc_rewards1" ~ "Credit_reward")))

#adding rows with regression info
(num_obs.vec = c("Observations", nrow(cash.reg$fit$model), NA, NA, nrow(credit.reg$fit$model), NA, NA, nrow(debit.reg$fit$model), NA, NA))
#
(loglikelihood.vec = c("Log Likelihood", round(logLik(cash.reg$fit),2), NA, NA, round(logLik(credit.reg$fit),2), NA, NA, round(logLik(debit.reg$fit),2), NA, NA))
#
(deviance.vec = c("Deviance", round(cash.reg$fit$deviance,2), NA, NA, round(credit.reg$fit$deviance,2), NA, NA, round(debit.reg$fit$deviance,2), NA, NA))
#
(aic.vec = c("AIC", round(AIC(cash.reg$fit),2), NA, NA, round(AIC(credit.reg$fit),2), NA, NA, round(AIC(debit.reg$fit),2), NA, NA))
#
(bic.vec = c("AIC", round(BIC(cash.reg$fit),2), NA, NA, round(BIC(credit.reg$fit),2), NA, NA, round(BIC(debit.reg$fit),2), NA, NA))

# row bind to reg.df
dim(reg2.df)
length(num_obs.vec)
(reg3.df = rbind(reg2.df, num_obs.vec, loglikelihood.vec, deviance.vec, aic.vec, bic.vec))

#export to LaTeX
print(xtable(reg2.df), hline.after =c(0,5), include.rownames=F)
...
# model
pi.model = PI ~ HH_income + Own_credit + Own_debit + Amount + Discount + Credit_reward
...
# plot prunned tree
prp(prune1, type = 3, box.palette = "auto", legend.x=NA, legend.y=NA, extra = 100, under = T, varlen = 0, faclen = 0, Margin = 0.0, digits = -2, tweak = 1.1, cex = 1.1, fallen.leaves = T, branch = 0.2)
...
#Begin: fraction PI by HH income####
names(m2025_8.df)
...
#
(frac_pi_by_income.df = data.frame(HH_income_group.vec, Cash = frac_cash.vec, Credit = frac_credit.vec, Debit = frac_debit.vec))

ggplot(frac_pi_by_income.df, aes(x=HH_income_group.vec, y=Cash, group = 1)) + geom_line(linetype="solid", linewidth=1.2, color="black") +geom_point(size=3, color="black")+geom_line(aes(y=Debit), linetype="longdash", linewidth=1.2, color="blue") +geom_point(aes(y=Debit), size=3, color="blue") +geom_line(aes(y=Credit), linetype="dotdash", linewidth=1.2, color="red") +geom_point(aes(y=Credit), size=3, color="red") +scale_x_discrete(breaks = seq(0,16,1)) + scale_y_continuous(breaks = seq(0, 1, 0.05), labels = percent)+labs(x="Annual household income group (low to high)", y="Preferred payment method (%)") +theme(axis.text.x = element_text(size = 14, color = "black"),  axis.text.y = element_text(size = 16, color = "black"), text = element_text(size = 20))  +annotate("text", x = 12, y = 0.54, angle=-17, label = "Debit card", size = 8, color="blue")  +annotate("text", x = 13, y = 0.22, label = "Cash", size = 8, color="black") +annotate("text", x = 11, y = 0.35, label = "Credit card", size = 8, color="red")

#End: fraction PI by HH income####
